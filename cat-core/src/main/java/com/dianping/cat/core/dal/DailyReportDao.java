...
public class DailyReportDao extends AbstractDao {
   ...
   public List<DailyReport> queryLatestReportsByDomainName(String domain, String name, int limits, Readset<DailyReport> readset) throws DalException {
      DailyReport proto = new DailyReport();

      proto.setDomain(domain);
      proto.setName(name);
      proto.setLimits(limits);

      List<DailyReport> result = getQueryEngine().queryMultiple(
            DailyReportEntity.QUERY_LATEST_REPORTS_BY_DOMAIN_NAME, 
            proto,
            readset);
