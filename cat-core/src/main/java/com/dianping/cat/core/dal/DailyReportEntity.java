...
public class DailyReportEntity {
   ...
   @Variable
   public static final DataField LIMITS = new DataField("limits");
   ...
   public static final QueryDef DELETE_BY_DOMAIN_NAME_PERIOD = new QueryDef("deleteByDomainNamePeriod", DailyReportEntity.class, QueryType.DELETE, 
      "DELETE FROM <TABLE/> WHERE <FIELD name='period'/> = ${period} AND <FIELD name='domain'/> = ${domain} AND <FIELD name='name'/> = ${name} AND binary(<FIELD name='domain'/>) = binary(${domain})");

   public static final QueryDef QUERY_LATEST_REPORTS_BY_DOMAIN_NAME = new QueryDef("queryLatestReportsByDomainName", DailyReportEntity.class, QueryType.SELECT, 
      "SELECT <FIELDS/> FROM <TABLE/> WHERE <FIELD name='domain'/> = ${domain} AND <FIELD name='name'/> = ${name} AND binary(<FIELD name='domain'/>) = binary(${domain}) ORDER BY <FIELD name='id'/> DESC LIMIT ${limits}");
